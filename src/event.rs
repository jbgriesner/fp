...
pub fn parse_event(action: &str, arg: Option<String>) -> Option<Event> {
    match action {
        ...
        "execute" => Some(Event::EvActExecute(
            arg.expect("execute event should have argument"),
        )),
        "execute-silent" => Some(Event::EvActExecuteSilent(
            arg.expect("execute-silent event should have argument"),
        )),
        "forward-char" => Some(Event::EvActForwardChar),
        "forward-word" => Some(Event::EvActForwardWord),
        "if-non-matched" => Some(Event::EvActIfNonMatched(
            arg.expect("no arg specified for event if-non-matched"),
        )),
        "if-query-empty" => Some(Event::EvActIfQueryEmpty(
            arg.expect("no arg specified for event if-query-empty"),
        )),
        "if-query-not-empty" => Some(Event::EvActIfQueryNotEmpty(
            arg.expect("no arg specified for event if-query-not-empty"),
        )),
        ...
        "half-page-down" => Some(Event::EvActHalfPageDown(
            arg.and_then(|s| s.parse().ok()).unwrap_or(1),
        )),
        "half-page-up" => Some(Event::EvActHalfPageUp(
            arg.and_then(|s| s.parse().ok()).unwrap_or(1),
        )),
