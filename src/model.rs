...
impl Model {
    pub fn new(
        ...
        let regex_engine: Rc<dyn MatchEngineFactory> = Rc::new(
            RegexEngineFactory::builder()
        ...
        let matcher = if let Some(engine_factory) = options.engine_factory.as_ref() {
            // use provided engine
            Matcher::builder(engine_factory.clone())
                .case(options.case)
                .build()
        } else {
            let fuzzy_engine_factory: Rc<dyn MatchEngineFactory> =
                Rc::new(AndOrEngineFactory::new(
                    ExactOrFuzzyEngineFactory::builder()
                        .exact_mode(options.exact)
                        .rank_builder(rank_builder.clone())
                        .build(),
                ));
            Matcher::builder(fuzzy_engine_factory)
                .case(options.case)
                .build()
        };

        let item_pool = Arc::new(DeferDrop::new(
            ItemPool::new().lines_to_reserve(options.header_lines),
        ));
    ...
    fn act_heart_beat(&mut self, env: &mut ModelEnv) {
        ...
        if matcher_stopped {
            let reader_stopped = self
                .reader_control
                .as_ref()
                .map(ReaderControl::is_done)
                .unwrap_or(true);
        ...
        if self.matcher_control.is_some() || !processed {
            ...
            let hb_timer_guard = self.timer.schedule_with_delay(
                TimerDuration::milliseconds(REFRESH_DURATION),
                move || {
    ...
    fn act_append_and_select(&mut self, env: &mut ModelEnv) {
        ...
        self.selection
            .act_select_matched(current_run_num(), matched_item);
        ...
        loop {
            ...
            match ev {
                ...
                Event::EvActIfNonMatched(ref arg_str) => {
                    let matched = self.num_options
                        + self
                            .matcher_control
                            .as_ref()
                            .map(|c| c.get_num_matched())
                            .unwrap_or(0);
                    if matched == 0 {
                ...
                Event::EvActDeleteCharEOF => {
                    if env.in_query_mode && env.query.is_empty()
